...
func (s *Store) UpsertLabel(id, name string, color int, deleted bool) error {
	_, err := s.db.Exec(`
		INSERT INTO labels(label_id, name, color, deleted)
		VALUES (?, ?, ?, ?)
		ON CONFLICT(label_id) DO UPDATE SET
			name = excluded.name,
			color = excluded.color,
			deleted = excluded.deleted
	`, id, name, color, boolToInt(deleted))
	return err
}

// Labels returns all non-deleted labels, ordered by numeric id then id.
